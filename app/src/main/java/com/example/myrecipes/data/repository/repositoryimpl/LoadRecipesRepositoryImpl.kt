...
import com.example.myrecipes.data.local.db.mapper.RECIPE_API_DATA_TO_VIEW_DATA_MAPPER
...
private val loadRecipesLocalDataSource: LoadRecipesLocalDataSource): LoadRecipesRepository {
    ...
    override fun loadRecipes(): Flow<Resource<List<RecipeViewData>>> = flow {
        ...
        if (response.isSuccessful && response.body()?.count != null /*&& response.body()?.count.toInt() > 0*/) {
            response.body()?.results?.let {
                emit(Resource.Success(RECIPE_API_DATA_TO_VIEW_DATA_MAPPER.map(it)))
            }
        } else {
