...
import com.example.myrecipes.data.local.db.mapper.LOADED_RECIPE_DATA_TO_ENTITY_MAPPER
...
class AddRecipesListUseCaseImpl(private val loadRecipesRepository: LoadRecipesRepository): AddRecipesListUseCase {
    override suspend fun invoke(recipes: List<LoadedRecipesData>) =
        loadRecipesRepository.addRecipes(LOADED_RECIPE_DATA_TO_ENTITY_MAPPER.map(recipes))
}